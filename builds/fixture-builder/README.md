...
* `getfile-stale-harness`stale-cache prompt + refresh;
  ...
  * `WARN_IF_CACHED_FILE_OLDER_THAN` and `file-edit-harness`, which exercises [`ensureShellRcRegion`](../../src/builder/shell-rc.sh), [`ensureWslConfIniLine`](../../src/builder/wsl-conf.sh), and [`ensureWslConfSectionLine`](../../src/builder/wsl-conf.sh) in Docker.
