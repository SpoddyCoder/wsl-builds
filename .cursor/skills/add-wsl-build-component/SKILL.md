...
9. **Optional** `wsl-builds.conf` keys (large downloads / durable caches the user may want on a host path): add **commented examples** to `wsl-builds.conf.example` and document usage in the build’s `README.md`. See `.cursor/rules/bash-component-patterns.mdc`.
   - Do **not** add version pins, source-mode switches, or installer version constants to `wsl-builds.conf.example` or user conf. Prefer interactive prompts at install time (e.g. stable release vs bleeding-edge git) or resolve/hardcode versions inside the install script. Only durable path, prefix, and cache keys belong in conf.
...
- If the user agrees to a **disable start-on-boot** prompt, follow `.cursor/rules/bash-component-patterns.mdc` (*Optional: disable start on boot*) and mirror `builds/ai/ollama/install.sh` (and multi-unit examples such as `builds/devops/docker/install.sh`): use `promptDisableSystemdUnitsOnBoot` or `isSystemdManagerRunning` from `src/builder/systemd-boot.sh`, confirm with `printInfo`; document wording in the build `README.md`.
