...
(`registerBuildMetadata` is defined once `conf.sh` is sourced from `./wsl-builder.sh`, which loads `src/builder/build-metadata.sh` first. The first argument is stored as `BUILD_DIR_NAME` and should match the build directory basename — e.g. `registerBuildMetadata "dev-rust" ...` for a `dev-rust/` dir.)
...
- `src/builder/arg-helpers.sh::showAvailableBuildDirs` scans `builds/*/` for dirs containing `conf.sh`, so the new dir appears in `./wsl-builder.sh` listings without further wiring.
...
- If the user asks for something that sounds like `fixture-builder` (noop components, CI-only), read [`builds/fixture-builder/README.md`](../../../builds/fixture-builder/README.md) and [`test/README.md`](../../../test/README.md) first. `fixture-builder` is **not** a README “Build List” build unless the maintainer explicitly asks to list it—it exists for Bats in Docker.
...
To add the first component, use the `add-wsl-build-component` skill and apply its messaging rules to the new `builds/<build-dir>/<slug>/install.sh`.
