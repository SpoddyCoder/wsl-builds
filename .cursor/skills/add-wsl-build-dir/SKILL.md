...
- If the user asks for something that sounds like **`test-fixture`** (noop components, CI-only), read [`test-fixture/README.md`](../../../test-fixture/README.md) and the **Testing** section in [`CONTRIBUTING.md`](../../../CONTRIBUTING.md) first. **`test-fixture`** is **not** a README “Build List” stack unless the maintainer explicitly asks to list it—it exists for Bats in Docker.
