...
# Docker-only: helper scripts under builds/system/ (apt-mirror-switch, change-hostname).
...
@test 'C1: apt-mirror-switch with no args prints usage and current mirror' {
	writeCanonicalUbuntuSources
	run env WSL_BUILDS_COMMAND_TEST_ROOT="${CMD_ROOT}" "${BUILDS_ROOT}/system/apt-mirror-switch"
	[[ "${status:?}" -eq 0 ]]
	[[ "${output:?}" =~ Usage: ]]
	[[ "${output:?}" =~ canonical ]]
	[[ "${output:?}" =~ uni-of-kent ]]
...
@test 'C3: apt-mirror-switch switches Kent ubuntu.sources to canonical (root)' {
	writeKentUbuntuSources
	run env WSL_BUILDS_COMMAND_TEST_ROOT="${CMD_ROOT}" "${BUILDS_ROOT}/system/apt-mirror-switch" canonical
	[[ "${status:?}" -eq 0 ]]
	[[ "${output:?}" =~ Current\ mirror:\ canonical ]]
...
@test 'C4: apt-mirror-switch switches canonical ubuntu.sources to uni-of-kent (root)' {
	writeCanonicalUbuntuSources
	run env WSL_BUILDS_COMMAND_TEST_ROOT="${CMD_ROOT}" "${BUILDS_ROOT}/system/apt-mirror-switch" uni-of-kent
	[[ "${status:?}" -eq 0 ]]
...
@test 'C5: apt-mirror-switch rejects unknown mirror' {
	writeCanonicalUbuntuSources
	run env WSL_BUILDS_COMMAND_TEST_ROOT="${CMD_ROOT}" "${BUILDS_ROOT}/system/apt-mirror-switch" not-a-mirror
	[[ "${status:?}" -eq 2 ]]
...
@test 'C6: apt-mirror-switch with too many args fails' {
	writeCanonicalUbuntuSources
	run env WSL_BUILDS_COMMAND_TEST_ROOT="${CMD_ROOT}" "${BUILDS_ROOT}/system/apt-mirror-switch" canonical extra
	[[ "${status:?}" -eq 1 ]]
}

@test 'C7: change-hostname with no args prints usage and fails' {
	run env WSL_BUILDS_COMMAND_TEST_ROOT="${CMD_ROOT}" "${BUILDS_ROOT}/system/change-hostname"
	[[ "${status:?}" -eq 1 ]]
	[[ "${output:?}" =~ Usage: ]]
}

@test 'C8: change-hostname updates wsl.conf and hosts under test root' {
	run env WSL_BUILDS_COMMAND_TEST_ROOT="${CMD_ROOT}" "${BUILDS_ROOT}/system/change-hostname" bats-test-host
	[[ "${status:?}" -eq 0 ]]
