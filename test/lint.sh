...
if [ "$#" -gt 0 ]; then
    ...
    for _lint_bash_file in "${REPO_ROOT}/build.sh" "${REPO_ROOT}/configure.sh" "${REPO_ROOT}/test/run-tests.sh" "${REPO_ROOT}/test/docker/run-bats.sh" "${REPO_ROOT}/test/lint.sh" "${REPO_ROOT}"/src/*.sh "${REPO_ROOT}"/*/install*.sh "${REPO_ROOT}"/*/conf.sh; do
