...
* After install, if **`systemd`** and **`/etc/systemd/system/ollama.service`** are present, you are prompted whether to **`systemctl disable --now ollama`** so the service stops immediately if running and does not start on boot (default **Y**). Choosing **n** leaves vendor enablement as-is.
